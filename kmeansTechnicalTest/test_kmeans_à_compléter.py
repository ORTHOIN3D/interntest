...
nbclust = ...
# KMeans algorithm:
kmeans = ... #To add random_state and remember to fit the data
...
for incentr in range(len(centroids)):
    for indtria in range(len(your_mesh.points)):
        ...
        if curr_dist <= 2:
            reduced_dist_list.append(current_normal_z)
            reduced_indx_list.append(indtria)
            reduced_color_list.append([color_vec[incentr][0], color_vec[incentr][1], color_vec[incentr][2]])
    
         
    tmpdict={incentr : {'nb_normals' : len(reduced_dist_list), 'normals' : reduced_dist_list, 'tria_indices' : reduced_indx_list, 'colors' : reduced_color_list}}
...
for inkey in range(len(belonging_classes)):
    ...
    for inpt in range(len(current_list_of_indices)):
        ...
        axes.scatter(your_mesh.points[current_list_of_indices[inpt]][0],
                    ...
                    c=current_color, s=1, label='centroid')
